    ...
    useEffect(() => {
        const fetchTimesheets = async () => {
            ...
            try {
                ...
                const mapped = (data.data || []).map((ts: Timesheet, idx: number) => {
                                    ...
                                    timesheets.map((ts) => (
                                        <tr key={ts.id}>
                                            ...
                                            <td className="px-4 py-4 bg-white border-b-1 border-[#e5e7eb]">
                                                <span className={`px-2 py-1 rounded-md text-xs font-semibold ${STATUS_STYLES[ts.status] || 'bg-gray-100 text-gray-500'}`}>{STATUS_LABELS[ts.status] || ts.status}</span>
                                            </td>
